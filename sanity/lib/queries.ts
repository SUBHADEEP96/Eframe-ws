...
const imageFields = `
  "url": asset->url,
  "alt": coalesce(alt, ^.accessibleLabel, ^.title),
  ...
  *[_type == "homepage"][0]{
    ...
    heroSlides[visible != false]
      | order(coalesce(order, 9999) asc){
          ...
          supportingText,
          mediaType,
          "desktopMedia": desktopMedia.asset->url,
          "mobileMedia": mobileMedia.asset->url,
          "poster": poster.asset->url,
          primaryCta,
          secondaryCta,
          "overlayStrength": coalesce(overlayStrength, 0.6)
        },
    sectionContent
  }
`);
