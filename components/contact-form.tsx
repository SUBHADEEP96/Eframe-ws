...
export function ContactForm() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus("We could not connect. Please email info@eframe.in directly.");
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <form onSubmit={submit} className="contact-form" noValidate>
      ...
      <label>
        ...
        />
        {field("message") && <small>{field("message")}</small>}
      </label>
      <label className="consent">
        <input type="checkbox" name="consent" />{" "}
        <span>
          I consent to Eframe using these details to respond to my enquiry. *
        </span>
      </label>
      {field("consent") && <small>{field("consent")}</small>}
      <label className="honeypot" aria-hidden="true">
        Website
        <input name="website" tabIndex={-1} autoComplete="off" />
      </label>
      <button className="cta-button" disabled={pending}>
