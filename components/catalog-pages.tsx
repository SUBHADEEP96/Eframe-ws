...
export function CatalogListing({title,eyebrow,description,items}:{title:string;eyebrow:string;description:string;items:CatalogItem[]}){
 return <InternalPage title={title} eyebrow={eyebrow} description={description} breadcrumbs={[{label:eyebrow}]} image={null}>
...
export function CatalogDetail({item,section}:{item:CatalogItem;section:string}){
 ...
 return <InternalPage title={item.title} eyebrow={item.eyebrow} description={item.summary} image={null} breadcrumbs={[{label:section,href:`/${section.toLowerCase().replaceAll(" ","-")}`},{label:item.title}]}>
